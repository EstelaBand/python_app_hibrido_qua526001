...
def main():                     # para não esquecer já faço o def main e faço o if name logo de cara
    limpar()
    while True:
        ...
        match opcao:
            ...
            case "1":
                ...
                print(f"x = {x}")
                continue
            case "2":      # esse vai calcular a equação de 2º grau
                a = float(input("Informe o valor de 'a': ").strip().replace(",","."))
                b = float(input("Informe o valor de 'b': ").strip().replace(",","."))
                c = float(input("Informe o valor de 'c': ").strip().replace(",","."))
                limpar()
                resultados = segundo_grau(a, b, c)
                for x in resultados:        # para ele exibir os resultados
                    print(f"x = {x}")
                continue
            case _:
                limpar()
